...
void main()
{
	...
	vs_out.FragPos = vec3(modelMat * vec4(vertexPos, 1.0)); // 转换为世界坐标，强制转成vec3，确保失去位移属性，以与vec3的光方向向量进行相减
	...
	vs_out.normalVector = mat3(transpose(inverse(modelMat))) * vNormalVectorPos; // 转换为世界坐标，法线矩阵与法向量相乘，修复不等比缩放，造成法向量不垂直，光照出错
	vs_out.texCoords = vTexturePos;
}